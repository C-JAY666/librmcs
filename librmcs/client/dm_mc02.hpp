...
class DMH7Board {
public:
    explicit DMH7Board(int32_t usb_pid = -1) {
        if (!init(0xa11c, usb_pid)) {
            throw std::runtime_error{"Failed to init usb transfer for DMH7Board, see log for detail."};
        }
    }
...
protected:
    ...
    virtual void can3_receive_callback(
        uint32_t can_id, uint64_t can_data, bool is_extended_can_id, bool is_remote_transmission,
        uint8_t can_data_length) {
        (void)can_id;
        (void)can_data;
    ...
    virtual void uart3_receive_callback(const std::byte* uart_data, uint8_t uart_data_length) {
        (void)uart_data;
        (void)uart_data_length;
    }
...
private:
    bool init(uint16_t vendor_id, int32_t product_id) noexcept {
        ...
        libusb_fill_bulk_transfer(
            ...
            [](libusb_transfer* transfer) {
                static_cast<DMH7Board*>(transfer->user_data)->usb_receive_complete_callback(transfer);
            },
            this, 0);
    ...
    void usb_receive_complete_callback(libusb_transfer* transfer) {
        ...
        while (iterator < sentinel) {
            ...
            if (field_id == UpwardId::CAN1) {
                read_can_buffer(iterator, &DMH7Board::can1_receive_callback);
            } else if (field_id == UpwardId::CAN2) {
                read_can_buffer(iterator, &DMH7Board::can2_receive_callback);
            } else if (field_id == UpwardId::CAN3) {
                read_can_buffer(iterator, &DMH7Board::can3_receive_callback);
            }else if (field_id == UpwardId::UART1) {
                read_uart_buffer(iterator, &DMH7Board::uart1_receive_callback);
            } else if (field_id == UpwardId::UART2) {
                read_uart_buffer(iterator, &DMH7Board::uart2_receive_callback);
            } else if (field_id == UpwardId::UART3) {
                read_uart_buffer(iterator, &DMH7Board::dbus_receive_callback);
            } else if (field_id == UpwardId::UART4) {
                read_uart_buffer(iterator, &DMH7Board::uart3_receive_callback);
    ...
    void read_can_buffer(std::byte*& buffer, decltype(&DMH7Board::can1_receive_callback) callback) {
    ...
    void read_uart_buffer(std::byte*& buffer, decltype(&DMH7Board::uart1_receive_callback) callback) {
...
class DMH7Board::TransmitBuffer final {
public:
    explicit TransmitBuffer(DMH7Board& DMH7Board, size_t alloc_transfer_count)
        : DMH7Board_(DMH7Board)
        ...
        free_transfers_.push_back_multi(
            [this, &DMH7Board]() {
                ...
                if (!transfer)
                    throw std::bad_alloc{};
                libusb_fill_bulk_transfer(
                    transfer, DMH7Board.libusb_device_handle_, out_endpoint, new unsigned char[64], 1,
                    [](libusb_transfer* transfer) {
    ...
    ~TransmitBuffer() {
        ...
        while (true) {
            ...
            if constexpr (utility::is_linux()) {
                ...
                ret = libusb_handle_events_timeout(DMH7Board_.libusb_context_, &timeout);
            } else {
                // Windows does not support timeout
                ret = libusb_handle_events(DMH7Board_.libusb_context_);
            }
            if (ret != 0) {
                LOG_ERROR(
    ...
    bool add_can3_transmission(
        uint32_t can_id, uint64_t can_data, bool is_extended_can_id = false,
        bool is_remote_transmission = false, uint8_t can_data_length = 8) {
        return add_can_transmission(
            DownwardId::CAN3, can_id, can_data, is_extended_can_id, is_remote_transmission,
            can_data_length);
    }
    ...
    bool add_uart3_transmission(const std::byte* uart_data, uint8_t uart_data_length) {
        return add_uart_transmission(DownwardId::UART4, uart_data, uart_data_length);
    }
